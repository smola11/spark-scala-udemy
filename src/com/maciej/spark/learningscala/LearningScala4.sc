object LearningScala4 {
  ...
  // Tuples (Also really common with Spark!!) - it is collection of stuff
  // Tuple is immutable list of objects that we cannot change once they have been created.
  ...
  // You refer to individual fields with their ONE-BASED index: (starts from 1 !!!)
 ...
 // Lists (in Scala they are IMMUTABLE)
...
// Dealing with missing keys; If we try to get non existing key from map - exception
println(shipMap.contains("Archer"))               //> false

// Similar concept to try/catch in Java
...
// go into more depth - but you've got enough to get through this course for now.
  ...
  val myNumberList = List.range(1, 21)
  println(myNumberList)

  for(number <- myNumberList){
    if (number % 3 == 0) println(number)
  }

  val divisibleByThree = myNumberList.filter(num => num % 3 == 0)
