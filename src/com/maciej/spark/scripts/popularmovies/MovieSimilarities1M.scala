...
object MovieSimilarities1M {
  ...
  def loadMovieNames() : Map[Int, String] = {
    ...
    var movieNames:Map[Int, String] = Map()
     ...
     val lines = Source.fromFile("movies.dat").getLines() // movies.dat will be distributed along with Spark driver script
     for (line <- lines) {
  ...
  def main(args: Array[String]) {
    ...
    // We want EMR's config defaults to be used. Fall back to the cluster configuration itself.
    ...
    val data = sc.textFile("s3n://sundog-spark/ml-1m/ratings.dat") // use Amazon S3 service (distributed file store with buckets)
    ...
    // This self-join is pretty expensive. partitionBy - we say explicitly 'I want you to take this operation and break up
    // into this many tasks. Spark breaks our DAG based on stages and stage is divided into individual tasks (distributed to nodes).
    // We should have at least as many partitions as executors.
    // The resulting RDD here will also have 100 partitions.
