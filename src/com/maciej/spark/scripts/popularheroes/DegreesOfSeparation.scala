...
object DegreesOfSeparation {
  ...
  // We will use this object to signal across the cluster if necessary when we found the person we're looking for
  // as we traverse our way through the graph
  ...
  def createStartingRdd(sc:SparkContext): RDD[BFSNode] = {
    val inputFile = sc.textFile("C:\\Users\\Właściciel\\Desktop\\Moje_duperele\\IT\\Coursera_Udemy\\Apache_Spark2_with_Scala-HandsOn_z_dużymi_danymi\\SparkScala\\marvel-graph.txt")
  ...
  def main(args: Array[String]) {
    ...
    hitCounter = Some(sc.longAccumulator("Hit Counter")) // now it is available to entire cluster at that point
    
    var iterationRdd = createStartingRdd(sc)
    
    var iteration:Int = 0
    for (iteration <- 1 to 10) { // 10 is an arbitrary number - an upper bound of the number of iterations that we will do
